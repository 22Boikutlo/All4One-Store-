...
namespace MyStore
{
    public partial class frmBeef : Form
    { 
        ...
        private void btnBuy_Click(object sender, EventArgs e)
        {
            ...
            {
                _CMD = new SqlCommand("SELECT Weight, Quantity FROM Products WHERE ProductId=2", dbConnection._CONNECT);
                ...
                _CMD = new SqlCommand("UPDATE Products SET Weight = @weight, Quantity = @quantity WHERE ProductId = 2", dbConnection._CONNECT);
                try
                {
        ...
        private void frmBeef_Load(object sender, EventArgs e)
        {
            _ConnectOpener.ConnectString();
            rad5.Checked = true;
            Boolean _boolCondition = _ConnectOpener.OpenConnect();
            ...
            if (_boolCondition == true)
            {
                _CMD = new SqlCommand("SELECT ProductName,ProductDescription,Weight,Price,Quantity FROM Products WHERE ProductId=2 ", dbConnection._CONNECT);
                _DTR = _CMD.ExecuteReader();
                if (_DTR.Read() == true)
                {
